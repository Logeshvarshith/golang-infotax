...
func TestNewUseCase_DeleteUserLoginDetail(t *testing.T) {
	t.Parallel()
	...
	t.Run("Db error occurred, while employee id exist check", func(t *testing.T) {
		...
		uc := NewUseCase(r)
		res, err := uc.DeleteUserLoginDetail(&ctx, id)
	...
	t.Run("Db error occurred, while deleting user login details", func(t *testing.T) {
		var ctx gin.Context
		var expected out.DeleteResponse
		r := repoMock.NewMockUserLoginDetailRepository(ctrl)
		r.EXPECT().CheckIfUserLoginDetailExists(gomock.AssignableToTypeOf(&ctx), validID).Return(true, nil)
		r.EXPECT().DeleteUserLoginDetail(gomock.AssignableToTypeOf(&ctx), validID).Return(fmt.Errorf("Db error occurred"))
		uc := NewUseCase(r)
		res, err := uc.DeleteUserLoginDetail(&ctx, vid)
		assert.Equal(t, err, error.NewInternal())
		assert.Equal(t, expected, res)
	})

	t.Run("Deleted user login details successfully", func(t *testing.T) {
		var ctx gin.Context
		...
		r := repoMock.NewMockUserLoginDetailRepository(ctrl)
		r.EXPECT().CheckIfUserLoginDetailExists(gomock.AssignableToTypeOf(&ctx), validID).Return(true, nil)
		r.EXPECT().DeleteUserLoginDetail(gomock.AssignableToTypeOf(&ctx), validID).Return(nil)
		uc := NewUseCase(r)
		res, err := uc.DeleteUserLoginDetail(&ctx, vid)
		assert.Nil(t, err)
		assert.Equal(t, expected, res)
