...
func TestUserLoginDetailHandler_DownloadUsersLoginDetail(t *testing.T) {
	...
	actualName := "outfile/user_login_detail.csv"
	t.Run("Failure, While giving invalid file path", func(t *testing.T) {
		...
		useCaser.EXPECT().DownloadUsersLoginDetail(gomock.AssignableToTypeOf(&ctx), actualName).Return("", "", expectedErr)
		h := NewUserLoginDetailHandler(useCaser)
		router := gin.Default()
		router.GET("/infotax/user_login_detail/download", h.DownloadUsersLoginDetail)

		w := httptest.NewRecorder()
		r, err := http.NewRequest("GET", "/infotax/user_login_detail/download", nil)
	...
	t.Run("Success, While giving valid file path", func(t *testing.T) {
		...
		name := "test.csv"
		useCaser := uldMock.NewMockUseCaser(ctrl)
		useCaser.EXPECT().DownloadUsersLoginDetail(gomock.AssignableToTypeOf(&ctx), actualName).Return(name, path, nil)
		h := NewUserLoginDetailHandler(useCaser)
		router := gin.Default()
		router.GET("/infotax/user_login_detail/download", h.DownloadUsersLoginDetail)
